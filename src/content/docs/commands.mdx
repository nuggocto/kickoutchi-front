...
description: A compact index of binaries, list/inspect/kill options, scoped kill flags, filters, JSON fields, and exit codes.
section: Reference
order: 1
mode: cli
related:
...
| `--tree`      | Linux/macOS only: terminate the target's whole descendant tree. Conflicts with `--group`.                  |
...
always re-collected fresh at execution time; stale previews are not trusted.

## `inspect`

Linux/macOS only. Show a process family without signalling anything.
...
| Option        | Description                                                                 |
| ------------- | --------------------------------------------------------------------------- |
| `--pid PID`   | Show this PID's ancestors, descendants, siblings, process group, and ports. |
